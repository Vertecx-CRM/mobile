...
class DashboardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (_) => SalesBloc(SalesRepository())..add(LoadSalesEvent()),
        ),
        BlocProvider(
          create: (_) =>
              ClientsBloc(ClientsRepository())..add(LoadClientsEvent()),
        ),
        BlocProvider(
          create: (_) =>
              PurchasesBloc(PurchasesRepository())..add(LoadPurchasesEvent()),
        ),
        BlocProvider(
          create: (_) =>
              AppointmentsBloc(AppointmentsRepository())
                ..add(LoadAppointmentsEvent()),
        ),
        BlocProvider(
          create: (_) => OrdersBloc(OrdersRepository())..add(LoadOrdersEvent()),
        ),
        BlocProvider(
          create: (_) =>
              ProductsBloc(ProductsRepository())..add(LoadProductsEvent()),
        ),
        BlocProvider(
          create: (_) =>
              CalendarBloc(AppointmentRepository())..add(LoadAllAppointments()),
        ),
      ],
      child: Scaffold(
                        ...
                        BlocBuilder<SalesBloc, SalesState>(
                          builder: (context, state) {
                            if (state is SalesLoaded) {
                              final total = state.sales.fold<double>(
                                0.0,
                                (sum, s) => sum + s.amount,
                              );
                        ...
                        BlocBuilder<PurchasesBloc, PurchasesState>(
                          builder: (context, state) {
                            if (state is PurchasesLoaded) {
                              final total = state.purchases.fold<double>(
                                0.0,
                                (sum, s) => sum + s.amount,
                              );
                              return SummaryCard(
                        ...
                        if (state is ClientsLoading) {
                          ...
                          return YearSalesChartWidget(
                            ...
                            sales: state.clients
                                .map(
                                  (c) =>
                                      Sales(month: c.month, amount: c.amount),
                                )
                                .toList(),
                            isClientChart: true,
                          );
                      ...
                      builder: (context, state) {
                        if (state is OrdersLoaded) {
                          return StateChartWidget(
                            title: "Órdenes",
                            description:
                                "Comparación de órdenes en pendientes, en proceso y completadas",
                            labels: [
                              "Completados",
                              "Pendientes",
                              "En progreso",
